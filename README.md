...
- **매출 트렌드 분석:** Olist의 과거(2016.09 ~ 2018.08) 패턴을 시간에 따른 GMV 변동을 보면 계절성, 소비 트렌드, 지역별 경제적 요인 등이 플랫폼에 미치는 영향을 파악가능. 이는 Olist가 미래 전략을 세우는 데 유용한 인사이트를 제공. 전체 국가 수준과 26개 주별로 평가하여 주문량, 평균 주문 금액(AOV)에 집중한 분석을 수행. 시간별 추이 - 특정 시즌에 매출이 증가하는 패턴 발견.
...
* **2017년 말 이후 매출액이 점점 감소하는 제품**
* **2017년 말 이후 매출액이 점점 증가하는 제품**
...
**매출액이 큰 비중을 차지하지만 주문이 감소하는 제품에 대해 프로모션 전략**
* 스포츠/레저와 "컴퓨터/악세서리" 제품라인은 매출액 4위로 전체 매출의 7%를 차지하지만 주문 및 평균 구매금액(AOV)이 감소 추세.
