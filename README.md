...
- **매출 트렌드 분석:** Olist의 과거(2016.09 ~ 2018.08) 패턴을 시간에 따른 GMV 변동을 보면 계절성, 소비 트렌드, 경제적 요인 등이 플랫폼에 미치는 영향을 파악가능. 이는 Olist가 미래 전략을 세우는 데 유용한 인사이트를 제공. 전체 국가 수준과 26개 주별로 평가하여 주문량, 평균 주문 금액(AOV)에 집중한 분석을 수행. 시간별 추이 - 특정 시즌에 매출이 증가하는 패턴 발견.
- **제품라인별 성과 분석:** 71개의 제품 라인들을 유사한 카테고리로 묶어 소비자들이 어떤 카테고리(제품군)을 구매하는 패턴, 전체 매출에 미치는 영향을 파악
<br>  
...
* Olist의 영업이익 산출 한계: Olist의 수익 구조은 판매자에게 플랫폼 수수료를 징수하는 비즈니스 모델. 즉 GMV는 판매자들이 플랫폼을 통해 판매한 전체 상품의 판매액 합계로 Olist의 직접적인 매출은 아니지만 연관성이 있고 플랫폼의 규모와 성장성을 보여주는 핵심 지표. 높은 GMV는 Olist가 많은 거래를 중개하고 있으며, 시장에서 경쟁력을 갖추고 있다는 뜻으로 이는 투자자나 내부 경영진이 Olist의 성과를 평가할 때 중요한 기준.
