...
# 가정 및 한계:
본 분석 과정에서 데이터 상의 한계 또는 문제점. 이를 해결하기 위해 여러 가정을 설정하였으며 이에 대한 가정과 한계의 세부사항은 아래와 같습니다.
* 데이터 정제 과정에서 누락값 및 오류가 포함된 행을 제거함에 따라 2016년 말의 주문건수 및 매출액이 실제와 상이할 수 있습니다.
* 영업이익 산출의 한계: 매출액은 계산 가능하나 플랫폼 수수료 구조 등 내부 정보가 부재하여 실제 기업 차원의 영업이익을 추정할 수 없습니다.
* 고객식별 한계: 고객 고유 ID 개수와 주문 건수가 동일하여 반복 구매 고객을 추적할 수 없습니다. 따라서 지역별 주문 수 분석은 해당 지역의 고유 고객 수를 반영합니다.
* 제품라인 정보 미비: 제품 라인의 구체적 구성(카테고리, 상품명 등)이 명시되지 않아 제품 단위 심층 분석이 불가능합니다.
